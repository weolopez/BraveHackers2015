...
.constant('datepickerConfig', {
    ...
    datepickerEvalAttributes: ['dateFormat', 'dayFormat', 'monthFormat', 'yearFormat', 'dayHeaderFormat', 'dayTitleFormat', 'disableWeekend', 'disableSunday', 'startingDay'],
...
.directive('attSearch', ["$document", "$filter", "$isElement", '$documentBind', function($document,$filter,$isElement,$documentBind){
    return{
        restrict: 'A',
        ...
        link: function(scope, element, attr, ctrl) {
            ...
            element.bind("keydown", function(e){                     
                if (e.which === KEY.BACKSPACE || e.which === KEY.ESC || e.which === KEY.ENTER || KEY.isControl(e.which) || KEY.isVerticalMovement(e.which) || KEY.isHorizontalMovement(e.which)) 
                {
                    e.preventDefault();
                    e.stopPropagation(); 
                ...
                {
                    if(scope.showSearch === false && e.keyCode !== 9 && e.keyCode !== 16)
                    { 
                        scope.showlist = true;
    ...
    return {
        ...
        link: function(scope,element,attr,ctrls){
            ...
            if(element.bind){
                element.bind('click', function(){
                    scope.clickHandler();
                    var emitId = iconStateConstants.SPLIT_ICON_BTN_EVENT_EMITTER_KEY;
                    scope.$broadcast(emitId, true);
                });
            }
...
angular.module('att.abs.tagBadges', [])
        .directive('tagBadges', ['$parse', function($parse) {
                return {
                    restrict: 'EA',
                    replace: false,
                    transclude: true,
...
angular.module("app/scripts/ng_js_att_tpls/datepicker/dateFilter.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("app/scripts/ng_js_att_tpls/datepicker/dateFilter.html",
    ...
    "            <input type=\"text\" ng-show=\"showCalendar\" att-input-deny=\"[^0-9\\/-]\" maxlength=\"{{maxLength}}\" ng-model=\"selectedOption\" ng-change=\"getDropdownText()\" />\n" +
    ...
    "            <input type=\"text\" ng-show=\"showCalendar\" att-input-deny=\"[^0-9\\/-]\" maxlength=\"{{maxLength}}\" ng-model=\"selectedOption\" ng-change=\"getDropdownText()\" />\n" +
    "            <abbr class=\"select2-search-choice-close\"></abbr>\n" +
    "            <span tabindex=\"0\" class=\"select2-arrow\" att-accessibility-click=\"13,32\" ng-click=\"showDropdown()\"><b></b></span>\n" +
    "        </a>\n" +
    "    </div>\n" +
    ...
    "        <ul class=\"select2-results\" style=\"margin-top:0px;max-height:320px;height:auto\">\n" +
    "            <li tabindex=\"0\" class=\"select2-result select2-highlighted\" ng-class=\"{'select2-result-current': checkCurrentSelection('Today')}\" att-accessibility-click=\"13,32\" ng-click=\"selectOption('Today', 0)\">\n" +
    "                <div class=\"select2-result-label\">Today</div>\n" +
    "            </li>\n" +
    "            <li tabindex=\"0\" class=\"select2-result select2-highlighted\" ng-class=\"{'select2-result-current': checkCurrentSelection('Past 7 Days')}\" att-accessibility-click=\"13,32\" ng-click=\"selectOption('Past 7 Days', 1)\">\n" +
    "                <div class=\"select2-result-label\">Past 7 Days</div>\n" +
    "            </li>\n" +
    "            <li tabindex=\"0\" class=\"select2-result select2-highlighted\" ng-class=\"{'select2-result-current': checkCurrentSelection('Past 15 Days')}\" att-accessibility-click=\"13,32\" ng-click=\"selectOption('Past 15 Days', 2)\">\n" +
    "                <div class=\"select2-result-label\">Past 15 Days</div>\n" +
    "            </li>\n" +
    "            <li tabindex=\"0\" class=\"select2-result select2-highlighted\" ng-class=\"{'select2-result-current': checkCurrentSelection('Past 30 Days')}\" att-accessibility-click=\"13,32\" ng-click=\"selectOption('Past 30 Days', 3)\">\n" +
    "                <div class=\"select2-result-label\">Past 30 Days</div>\n" +
    "            </li>\n" +
    ...
    "                <div class=\"select2-result-label\">Custom Single Date...</div>\n" +
    "            </li>\n" +
    ...
    "                <div class=\"select2-result-label\">Custom Range...</div>\n" +
    "            </li>\n" +
    "            <li class=\"select2-result select2-highlighted btnContainer\" ng-style=\"{display: (showCalendar && 'block') || 'none'}\">\n" +
    "                <button tabindex=\"0\" att-button=\"\" btn-type=\"{{applyButtonType}}\" size=\"small\" att-accessibility-click=\"13,32\" ng-click=\"apply()\">Apply</button>\n" +
    "                <button tabindex=\"0\" att-button=\"\" btn-type=\"secondary\" size=\"small\" att-accessibility-click=\"13,32\" ng-click=\"cancel()\">Cancel</button>\n" +
    "                <div><a tabindex=\"0\" href=\"javascript:void(0)\" style=\"text-decoration:underline;\" att-accessibility-click=\"13,32\" ng-click=\"clear()\">Clear Dates</a>\n" +
    "                </div>\n" +
    "            </li>\n" +
    "        </ul>\n" +
    "    </div>\n" +
    "    <div class=\"datepicker-wrapper show-right\" ng-style=\"{display: (showCalendar && 'block') || 'none'}\">\n" +
    "        <span datepicker ng-show=\"checkCurrentSelection('Custom Single Date')\"></span>\n" +
    "        <span datepicker ng-show=\"checkCurrentSelection('Custom Range')\"></span>\n" +
    "    </div>\n" +
    "</div>\n" +
    "");
}]);
...
angular.module("app/scripts/ng_js_att_tpls/datepicker/datepicker.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("app/scripts/ng_js_att_tpls/datepicker/datepicker.html",
    "<ul id=\"datepicker\" class=\"datepicker\" aria-hidden=\"false\" role=\"dialog\" tabindex=\"-1\">\n" +
    "    <div class=\"datepicker-days\" style=\"display: block;\">\n" +
    "        <table class=\"table-condensed\">\n" +
    "            <thead>\n" +
    "                <tr>\n" +
    "                    <th id=\"month\" tabindex=\"0\" align=\"left\" class=\"datepicker-switch\" colspan=\"{{currentRows[0].length - 2}}\">{{currentTitle}}</th>\n" +
    "                    <th id=\"prev\" tabindex=\"{{!disablePrev && '0' || '-1'}}\" att-accessibility-click=\"13,32\" ng-click=\"!disablePrev && move(-1)\">\n" +
    ...
    "                    </th>\n" +
    "                    <th id=\"next\" tabindex=\"{{!disableNext && '0' || '-1'}}\" att-accessibility-click=\"13,32\" ng-click=\"!disableNext && move(1)\">\n" +
    ...
    "                        <table class=\"table-condensed\" style=\"padding: 0px;\">\n" +
    "                            <tbody>\n" +
    "                                <tr ng-repeat=\"row in currentRows\">\n" +
    "                                    <td headers=\"{{dt.header}}\" att-element-focus=\"dt.focused\" tabindex=\"{{(!dt.oldMonth && !dt.nextMonth && '0') || '-1'}}\" ng-repeat=\"dt in row\" class=\"days\" ng-class=\"{'active': dt.selected || dt.from || dt.to, 'from': dt.from, 'to': dt.to, 'range': dt.dateRange, 'prev-month ': dt.oldMonth, 'next-month': dt.nextMonth, 'disabled': dt.disabled, 'today': dt.today, 'weekend': dt.weekend}\" ng-click=\"!dt.selected && !dt.from && !dt.to && !dt.disabled && !dt.oldMonth && !dt.nextMonth && select(dt.date)\" att-accessibility-click=\"13,32\"><span class=\"day\">{{dt.label}}</span></td>\n" +
    "                                </tr>\n" +
    "                                <tr>\n" +
...
angular.module("app/scripts/ng_js_att_tpls/hourpicker/hourpicker.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("app/scripts/ng_js_att_tpls/hourpicker/hourpicker.html",
    ...
    "                    <div ng-model=\"FrtimeListDay[day]\" class=\"select2-container topDropDownWidth\" ng-class=\"{'select2-dropdown-open select2-container-active': FrtimeListDay[day]}\" >\n" +
    ...
    "                \n" +
    "                <div class=\"dropDownMarginBottom\">\n" +
    "                    <div ng-model=\"TotimeListDay[day]\" class=\"select2-container topDropDownWidth\" ng-class=\"{'select2-dropdown-open select2-container-active': TotimeListDay[day]}\" >\n" +
...
angular.module("app/scripts/ng_js_att_tpls/search/search.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("app/scripts/ng_js_att_tpls/search/search.html",
    ...
    "	<a href=\"#\" class=\"select2-choice needsclick\" tabindex=\"0\" ng-click=\"showDropdown()\" ng-class=\"{'select2-chosen-disabled':isDisabled}\" ng-focus=\"isact=true;\" ng-blur=\"isact=false;\">           \n" +
...
angular.module("app/scripts/ng_js_att_tpls/select/textDropdown.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("app/scripts/ng_js_att_tpls/select/textDropdown.html",
    ...
    "		<li ng-class=\"{'highlight': selectedIndex==$index}\" ng-repeat=\"action in actions track by $index\" ng-click=\"chooseAction($event, action)\">{{action}}<i ng-class=\"{'icon-included-checkmark': isCurrentAction(action)}\"></i></li>\n" +
    "	</ul>\n" +
    "\n" +
    "</div>");
}]);
...
angular.module("app/scripts/ng_js_att_tpls/splitButtonDropdown/splitIconButton.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("app/scripts/ng_js_att_tpls/splitButtonDropdown/splitIconButton.html",
    "<div att-accessibility-click=\"13,32\">\n" +
...
angular.module("app/scripts/ng_js_att_tpls/table/attTableHeader.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("app/scripts/ng_js_att_tpls/table/attTableHeader.html",
    "<th tabindex=\"{{sortable !== 'false' && '0' || '-1'}}\" class=\"tablesorter-header\" ng-class=\"{'tablesorter-headerAsc': sortPattern === 'asc', 'tablesorter-headerDesc': sortPattern === 'desc', 'tablesort-sortable': sortable !== 'false', 'sorter-false': sortable === 'false'}\" att-accessibility-click=\"13,32\" ng-click=\"(sortable !== 'false') && sort()\"><div class=\"tablesorter-header-inner\" ng-transclude></div></th>\n" +
    "");
}]);
...
angular.module("app/scripts/ng_js_att_tpls/tabs/floatingTabs.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("app/scripts/ng_js_att_tpls/tabs/floatingTabs.html",
    ...
    " <li ng-repeat=\"tab in tabs\" ng-class=\"{'tabsbid__item tabsbid__item--active': isActiveTab(tab.url), 'tabsbid__item': !isActiveTab(tab.url)}\" ng-click=\"onClickTab(tab)\" att-accessibility-click=\"32,13\">\n" +
    "        <a class=\"tabsbid__item-link\" href=\"{{tab.url}}\" tabindex=\"0\">{{tab.title}}</a>\n" +
    "  </li>\n" +
    "</ul>");
}]);
...
angular.module("app/scripts/ng_js_att_tpls/tagBadges/tagBadges.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("app/scripts/ng_js_att_tpls/tagBadges/tagBadges.html",
    ...
    "    <span class=\"tags__item--title\" ng-mousedown=\"activeHighlight(true)\" ng-mouseup=\"activeHighlight(false)\" ng-transclude></span>\n" +
...
angular.module("app/scripts/ng_js_att_tpls/toggle/demoToggle.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("app/scripts/ng_js_att_tpls/toggle/demoToggle.html",
    ...
    "    <div class=\"att-switch-thumb\" ng-class=\"{'large' : directiveValue == 'large'}\"></div>\n" +
...
angular.module("app/scripts/ng_js_att_tpls/videoControls/photoControls.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("app/scripts/ng_js_att_tpls/videoControls/photoControls.html",
    "<div>\n" +
    "    <a ng-href=\"{{prevLink}}\"><i class=\"icon-arrow-left\">&nbsp;</i></a>\n" +
    "    <span ng-transclude></span>\n" +
    "    <a ng-href=\"{{nextLink}}\"><i class=\"icon-arrow-right\">&nbsp;</i></a>\n" +
    "</div>");
}]);
